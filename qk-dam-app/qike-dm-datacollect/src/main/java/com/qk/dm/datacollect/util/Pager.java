...
public class Pager<T> {
  ...
  public static <T> List<T> getList(Pagination pagination, List<T> data) {
    return getList(pagination.getPage(),pagination.getSize(),data);
  }

  public static <T> List<T> getList(int page, int perPage, List<T> data) {
    if(CollectionUtils.isEmpty(data)){
      return new ArrayList<>();
    }
    Pager<T> pager = Pager.create(data, perPage);
    return pager.getPagedList(page);
  }
