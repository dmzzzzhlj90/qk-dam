...
/**
 * An implementation of a {@link JwtEncoder} that encodes a JSON Web Token (JWT) using the JSON Web
 * Signature (JWS) Compact Serialization format. The private/secret key used for signing the JWS is
 * supplied by the {@code com.nimbusds.jose.jwk.source.JWKSource} provided via the constructor.
 *
 * <p><b>NOTE:</b> This implementation uses the Nimbus JOSE + JWT SDK.
 *
 * @author Joe Grandja
 * @since 0.0.1
 * @see JwtEncoder
 ...
 * @see <a target="_blank" href="https://tools.ietf.org/html/rfc7515#section-3.1">JWS Compact
 *     Serialization</a>
 * @see <a target="_blank" href="https://connect2id.com/products/nimbus-jose-jwt">Nimbus JOSE + JWT
 *     SDK</a>
 */
  ...
  private static class JwsHeaderConverter implements Converter<JoseHeader, JWSHeader> {
    ...
    public JWSHeader convert(JoseHeader headers) {
      ...
      if (!CollectionUtils.isEmpty(jwk)) {
        try {
          builder.jwk(JWK.parse(String.valueOf(jwk)));
        } catch (Exception ex) {
          throw new JwtEncodingException(
              String.format(
                  ENCODING_ERROR_MESSAGE_TEMPLATE,
