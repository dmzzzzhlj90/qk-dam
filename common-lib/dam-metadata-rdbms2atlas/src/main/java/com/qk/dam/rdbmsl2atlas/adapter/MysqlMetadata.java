...
public class MysqlMetadata {
    ...
    public AtlasEntity.AtlasEntitiesWithExtInfo syncMysqlMetadata(List<MysqlDbType> mysqlDbTypes) throws Exception{
        var entitiesWithExtInfo = new AtlasEntity.AtlasEntitiesWithExtInfo();
        mysqlDbTypes.stream().map(this::convertAtlasEntity).flatMap(Collection::stream).forEach(entitiesWithExtInfo::addEntity);
    ...
    private List<AtlasEntity> convertAtlasEntity(MysqlDbType mysqlDbType) {
        ...
        mysqlDbType
                .getMysqlTableTypes().stream()
                .map(this::convertAtlasEntity)
                .forEach(mysqlTableType -> {
    ...
    private MysqlTableType convertAtlasEntity(MysqlTableType mysqlTableType) {
        ...
        Map<String, Object> tbAttr =
                OBJECT_MAPPER.convertValue(
                        mysqlTableType,
                        TYPE_FACTORY.constructMapType(Map.class, String.class, Object.class));
        final var table = MysqlTypeEnum.MYSQL_TABLE.getAtlasEntity();
        table.setAttributes(tbAttr);

        if (CollectionUtils.isNotEmpty(mysqlTableType
                .getMysqlColumnTypes())){
            List<AtlasEntity> entities = mysqlTableType
                    .getMysqlColumnTypes().stream()
                    .map(this::convertAtlasEntity)
                    .map(mysqlColumnType -> {
                        // 配置列与表的关系
                        AtlasEntity columnEntity = mysqlColumnType.getColumnEntity();
                        columnEntity.setRelationshipAttribute(
                                MysqlRelationshipTypeEnum.TABLE.getName(),
                                toAtlasRelatedObjectId(table));
                        // 返回实体
                        return columnEntity;
