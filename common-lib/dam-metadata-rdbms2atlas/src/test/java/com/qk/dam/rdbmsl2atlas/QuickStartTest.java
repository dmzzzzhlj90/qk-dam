...
//
//        AtlasSearchResult mysql_db = quick.atlasClientV2.dslSearchWithParams("from mysql_db", 40, 0);
//        Set<String> guids = mysql_db.getEntities().stream().map(AtlasEntityHeader::getGuid).collect(Collectors.toSet());
//        quick.atlasClientV2.purgeEntitiesByGuids(guids);
//
//        AtlasSearchResult mysql_table = quick.atlasClientV2.dslSearchWithParams("from mysql_table", 40, 0);
//        Set<String> tguids = mysql_db.getEntities().stream().map(AtlasEntityHeader::getGuid).collect(Collectors.toSet());
//        quick.atlasClientV2.purgeEntitiesByGuids(tguids);
//
//        AtlasSearchResult mysql_column = quick.atlasClientV2.dslSearchWithParams("from mysql_column", 40, 0);
//        Set<String> cguids = mysql_db.getEntities().stream().map(AtlasEntityHeader::getGuid).collect(Collectors.toSet());
