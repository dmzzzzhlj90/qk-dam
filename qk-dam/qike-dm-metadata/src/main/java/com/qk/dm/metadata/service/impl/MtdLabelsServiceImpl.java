...
public class MtdLabelsServiceImpl implements MtdLabelsService {
    ...
    public MtdLabelsServiceImpl(EntityManager entityManager, MtdLabelsRepository mtdLabelsRepository, MtdLabelsAtlasRepository mtdLabelsAtlasRepository) {
    ...
    public List<MtdLabelsAtlas> synchLabels(List<MtdLabels> labelAllList) {
        ...
        return labelsAtlases.stream()
                .filter(i -> CollectionUtils.containsAny(labelsNameList, Arrays.asList(i.getLabels().split(","))))
                .peek(mla -> {
                    String labels = Stream.of(mla.getLabels().split(",")).filter(y -> !labelsNameList.contains(y)).collect(Collectors.joining(","));
