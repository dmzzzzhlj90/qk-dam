...
public class DisRuleClassServiceImpl implements DisRuleClassService {
    ...
    public DisRuleClassVO list() {
        ...
        return DisRuleClassVO.builder().parentId(Long.parseLong("-1")).id(Long.parseLong("-1"))
                .title("跟目录").value("根目录").children(childrenList).build();
    }

    @Override
    public void insert(DisRuleClassVO vo) {
        Predicate predicate = qDisRuleClassification.classificationName.eq(vo.getTitle())
        ...
        disRuleClassificationRepository.save( DisRuleClassMapper.INSTANCE.of(vo));
    }
    ...
    public void delete(String id) {
       ...
       if(Objects.isNull(disRuleClassification)){
        ...
        deleteChildren(disRuleClassification.getId());
    }

    private void deleteChildren(Long id){
        disRuleClassificationRepository.deleteById(id);
       List<DisRuleClassification>  childrenList = disRuleClassificationRepository.findAllByParentId(id);
        if(CollectionUtils.isEmpty(childrenList)){
            return;
        }
        childrenList.forEach(e->deleteChildren(e.getId()));
    }
