...
public class CosTaskFilesServiceImpl implements CosTaskFilesService {
    ...
    public void setFiLesMetaDataHearderInfo(String dataDay) {
        ...
        Map<String, PiciTaskVO> piciTaskVOMap = piciTaskVOS.stream().collect(Collectors.toMap(PiciTaskVO::getOssPath, PiciTaskVO -> PiciTaskVO));
        int index = 0;
        for (COSObjectSummary cosObjectSummary : cosObjectSummaryList) {
            if (index != 0) {
                ...
                ObjectMetadata objectMetadata = cosClient.getObjectMetadata(LongGovConstant.BUCKETNAME, key);
