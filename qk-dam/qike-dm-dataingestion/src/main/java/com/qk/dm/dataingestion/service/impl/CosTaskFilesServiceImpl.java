    ...
    public List<CosTaskFileInfoVO> getCosTaskFilesInfo(String dataDay) {
        ...
        try {
            ...
            if (result.size() > 0) {
                ...
                List<String> tableNamePiciList = piciIsHiveAllList.stream().map(ptlv -> ptlv.getTableName() + "_" + ptlv.getPici()).collect(Collectors.toList());
                result = result.stream().filter(ctfiv -> tableNamePiciList.contains(ctfiv.getTableName() + "_" + ctfiv.getPici())).collect(Collectors.toList());
