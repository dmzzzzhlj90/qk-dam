...
public class DataStandardCodeInfoServiceImpl implements DataStandardCodeInfoService {
    ...
    public Map<String, Object> getDsdCodeInfoExt(DsdCodeInfoExtParamsVO dsdCodeInfoExtParamsVO) {
        ...
        List<CodeTableFieldsVO> codeTableFieldsVOList = GsonUtil.
                fromJsonString(tableConfFieldsStr, new TypeToken<List<CodeTableFieldsVO>>() {
                }.getType());
    ...
    public void extCheckCondition(
        ...
        if (!StringUtils.isEmpty(dsdCodeInfoExtParamsVO.getTableConfCode())) {
            booleanBuilder.and(
                    qDsdCodeInfoExt.tableConfCode.contains(dsdCodeInfoExtParamsVO.getTableConfCode()));
        }
        if (!StringUtils.isEmpty(dsdCodeInfoExtParamsVO.getTableConfValue())) {
            booleanBuilder.and(
                    qDsdCodeInfoExt.tableConfValue.eq(dsdCodeInfoExtParamsVO.getTableConfValue()));
        }
    ...
    public void addDsdCodeInfoExt(DsdCodeInfoExtVO dsdCodeInfoExtVO) {
        ...
        if (exists) {
            throw new BizException(
                    "当前要新增的码表数值,编码code为:" + dsdCodeInfoExt.getTableConfCode() + " 的数据，已存在！！！");
        }
    ...
    public void modifyDsdCodeInfoExt(DsdCodeInfoExtVO dsdCodeInfoExtVO) {
        ...
        if (exists) {
            ...
            throw new BizException(
                    "当前要新增的码表数值,编码code为:" + dsdCodeInfoExt.getTableConfCode() + " 的数据，不存在！！！");
        }
    }
