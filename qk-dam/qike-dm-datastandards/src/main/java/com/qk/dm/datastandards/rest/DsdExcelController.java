...
public class DsdExcelController {
  ...
  /**
   * 业务术语excel__全部导出数据 @Param: response
   *
   * @return: void
   */
  ...
  public DefaultCommonResult basicInfoUploadByDirDsdId(
      MultipartFile file, @RequestParam("dirDsdId") String dirDsdId) throws IOException {
    dsdExcelService.basicInfoUpload(file, dirDsdId);
    return new DefaultCommonResult(ResultCodeEnum.OK);
  }

  /**
   * 码表信息excel__全部导出数据 @Param: response
   *
   * @return: void
   */
  ...
  /**
   * 码表信息excel__导出数据 (根据码表分类目录Id) @Param: response
   *
   * @return: void
   */
  @PostMapping("/code/term/download/codeDirId")
  public void codeTermDownloadByCodeDir(
      HttpServletResponse response, @RequestParam("codeDirId") String codeDirId)
      throws IOException {
  ...
  /**
   * 码表信息excel_导入数据(根据码表分类目录Id) @Param: file
   *
   * @return: java.lang.String
   */
  @PostMapping("/code/term/upload/codeDirId")
  @ResponseBody
  public DefaultCommonResult codeTermUploadByCodeDir(
      MultipartFile file, @RequestParam("codeDirId") String codeDirId) throws IOException {
    dsdExcelService.codeTermUpload(file, codeDirId);
    return new DefaultCommonResult(ResultCodeEnum.OK);
  }

  /**
   * 码表信息excel__导入数据(默认根据Excel中选择的码表层级进行导入) @Param: file
   *
   * @return: java.lang.String
   */
  ...
  /**
   * 码表信息excel__模板下载 @Param: response
   *
   * @return: void
   */
  ...
  public void codeTermDownloadTemplate(HttpServletResponse response) throws IOException {
    ...
    EasyExcel.write(response.getOutputStream(), DsdCodeTermVO.class)
        .registerWriteHandler(new DsdCodeTermCustomSheetWriteHandler(dsdExcelService))
        .sheet("模板")
        .doWrite(codeTermSampleDataList);
  }
}
