...
public class DataStandardCodeTermServiceImpl implements DataStandardCodeTermService {
  ...
  @Autowired
  public DataStandardCodeTermServiceImpl(
  ...
  public PageResultVO<DsdCodeTermVO> getDsdCodeTerm(DsdCodeTermParamsVO dsdCodeTermParamsVO) {
    ...
    list.forEach(
        dsdCodeTerm -> {
          ...
          dsdCodeTermVO.setCodeDirLevelName(
              codeDirLevel.split("/")[codeDirLevel.split("/").length - 1]);
          dsdCodeTermVOList.add(dsdCodeTermVO);
        });
    return new PageResultVO<>(
        total,
        dsdCodeTermParamsVO.getPagination().getPage(),
        dsdCodeTermParamsVO.getPagination().getSize(),
        dsdCodeTermVOList);
  }
  ...
  public void bulkAddDsdCodeTerm(List<DsdCodeTermVO> dsdCodeTermVOList) {
    Set<String> codeIds =
        dsdCodeTermVOList.stream()
            .map(dsdCodeTermVO -> dsdCodeTermVO.getCodeId())
            .collect(Collectors.toSet());
    List<DsdCodeTerm> searchResult =
        (List<DsdCodeTerm>)
            dsdCodeTermRepository.findAll(QDsdCodeTerm.dsdCodeTerm.codeId.in(codeIds));

    if (searchResult.size() > 0) {
      throw new BizException(
          "当前要新增的码表编码ID为："
              + searchResult.get(0).getCodeId()
              + "码表名称名称为:"
              + searchResult.get(0).getCodeName()
              + " 的数据，已存在！！！");
    }

    List<DsdCodeTerm> dsdCodeTermList = new ArrayList<>();
    dsdCodeTermVOList.forEach(
        dsdCodeTermVO -> {
  ...
  public void bulkDeleteDsdTerm(String ids) {
    ...
    Iterable<DsdCodeTerm> dsdCodeTerms =
        dsdCodeTermRepository.findAll(QDsdCodeTerm.dsdCodeTerm.id.in(idSet));
    dsdCodeTermRepository.deleteInBatch(dsdCodeTerms);
  }

  public Map<String, Object> queryDsdCodeTermByParams(DsdCodeTermParamsVO dsdCodeTermParamsVO)
    ...
    long count =
        jpaQueryFactory
            .select(qDsdCodeTerm.count())
            .from(qDsdCodeTerm)
            .where(booleanBuilder)
            .fetchOne();
    List<DsdCodeTerm> dsdCodeTermList =
        jpaQueryFactory
            .select(qDsdCodeTerm)
            .from(qDsdCodeTerm)
            .where(booleanBuilder)
            ...
            .offset(
                (dsdCodeTermParamsVO.getPagination().getPage() - 1)
                    * dsdCodeTermParamsVO.getPagination().getSize())
  ...
  public void checkCondition(
      ...
      QDsdCodeTerm qDsdCodeTerm,
      DsdCodeTermParamsVO dsdCodeTermParamsVO) {
    if (!StringUtils.isEmpty(dsdCodeTermParamsVO.getCodeDirId())
        && !DsdConstant.TREE_DIR_TOP_PARENT_ID.equals(dsdCodeTermParamsVO.getCodeDirId())) {
    ...
    if (!StringUtils.isEmpty(dsdCodeTermParamsVO.getBeginDay())
        && !StringUtils.isEmpty(dsdCodeTermParamsVO.getEndDay())) {
      StringTemplate dateExpr =
          Expressions.stringTemplate(
              "DATE_FORMAT({0},'%Y-%m-%d %H:%i:%S')", qDsdCodeTerm.gmtModified);
      booleanBuilder.and(
          dateExpr.between(dsdCodeTermParamsVO.getBeginDay(), dsdCodeTermParamsVO.getEndDay()));
    }
  }
}
