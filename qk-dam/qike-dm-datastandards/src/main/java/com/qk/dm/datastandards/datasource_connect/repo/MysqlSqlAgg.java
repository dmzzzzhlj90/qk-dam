...
public class MysqlSqlAgg {
  ...
  private MysqlDb getMysqlDbType(
      String database, List<Entity> entityTableList, List<Entity> entityColumnList) {
    // DB
  ...
  private List<DsdCodeInfoExt> getCodeValues(
      List<Entity> entityList, MysqlTable mysqlTable, Long codeInfoId) {
    if (entityList != null && entityList.size() > 0) {
      List<MysqlColumn> mysqlColumnList = mysqlTable.getMysqlColumns();
      List<String> colKeyList =
          mysqlColumnList.stream().map(MysqlColumn::getColName).collect(Collectors.toList());
