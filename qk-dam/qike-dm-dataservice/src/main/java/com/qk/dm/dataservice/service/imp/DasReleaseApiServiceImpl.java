...
public class DasReleaseApiServiceImpl implements DasReleaseApiService {
  ...
  public void apiSixConsumersKeyAuth(ApiSixConsumerInfo apiSixConsumerInfo) {
    ...
    systemParam.put(
        ApiSixConstant.API_SIX_ADMIN_CONSUMER_URL_KEY, apiSixConnectInfo.getAdminConsumerUrl());
  ...
  public List apiSixUpstreamInfo() {
    ...
    Map<String, String> systemParam = new HashMap<>();
    systemParam.put(
        ApiSixConstant.API_SIX_ADMIN_UPSTREAM_URL_KEY, apiSixConnectInfo.getAdminUpstreamUrl());
    systemParam.put(ApiSixConstant.API_SIX_HEAD_KEY, apiSixConnectInfo.getHeadKeyValue());
    upstreamContext.setParams(systemParam);
    List upstreamInfoList =
        apiGatewayManager.getUpstreamInfo(ApiSixConnectInfo.GATEWAY_TYPE_API_SIX, upstreamContext);
  ...
  private int singleSyncRegisterApi(Map<String, List<DasApiBasicInfo>> apiBasicInfoMap, String upstreamId, String serviceId) {
    ...
    try {
      ...
      for (DasApiRegister dasApiRegister : dasApiRegisterIterable) {
        ...
        boolean flag = setRouteInfo(dasApiBasicInfo, upstreamId, serviceId, routeContext, dasApiRegister);
        updateApiRegisterStatus(dasApiBasicInfo, dasApiRegister, flag, successfulNum, failNum);
      }
  ...
  private boolean setRouteInfo(DasApiBasicInfo dasApiBasicInfo,
                               String upstreamId,
                               String serviceId,
    ...
    boolean flag = false;
    try {
      ApiSixRouteInfo apiSixRouteInfo = getApiSixRouteInfo(dasApiBasicInfo, upstreamId, serviceId, dasApiRegister);
  ...
  private int singleSyncRegisterApiByPath(Map<String, List<DasApiBasicInfo>> apiBasicInfoMap,
    ...
    try {
      ...
      Iterable<DasApiRegister> dasApiRegisterIterable = dasApiRegisterRepository
              .findAll(qDasApiRegister.backendPath.contains(apiPath).and(qDasApiRegister.apiId.in(apiBasicInfoMap.keySet())));
      for (DasApiRegister dasApiRegister : dasApiRegisterIterable) {
        DasApiBasicInfo dasApiBasicInfo = apiBasicInfoMap.get(dasApiRegister.getApiId()).get(0);
        boolean flag = setRouteInfoByPath(dasApiBasicInfo, upstreamId, serviceId, routeContext, routeIdList, dasApiRegister);
        updateApiRegisterStatus(dasApiBasicInfo, dasApiRegister, flag, successfulNum, failNum);
      }
      LOG.info("成功同步注册Api个数:【{}】", successfulNum);
      LOG.info("失败同步注册Api个数:【{}】", failNum);
  ...
  private void updateApiRegisterStatus(DasApiBasicInfo dasApiBasicInfo,
                                       DasApiRegister dasApiRegister,
                                       boolean flag,
  ...
  private boolean setRouteInfoByPath(DasApiBasicInfo dasApiBasicInfo,
                                     String upstreamId, String serviceId,
                                     RouteContext routeContext, List<String> routeIdList,
                                     DasApiRegister dasApiRegister) {
    boolean flag = false;
    try {
      // 清除匹配到的Route
      ...
      flag = setRouteInfo(dasApiBasicInfo, upstreamId, serviceId, routeContext, dasApiRegister);
  ...
  private RouteContext buildRouteContext() {
    ...
    Map<String, String> systemParam = new HashMap<>();
    systemParam.put(
        ApiSixConstant.API_SIX_ADMIN_ROUTE_URL_KEY, apiSixConnectInfo.getAdminRouteUrl());
    systemParam.put(ApiSixConstant.API_SIX_HEAD_KEY, apiSixConnectInfo.getHeadKeyValue());
    routeContext.setParams(systemParam);
    return routeContext;
  }

  private void deleteRouteByRouteId(
      RouteContext routeContext, DasApiRegister dasApiRegister, List<String> routeIdList) {
  ...
  private ApiSixRouteInfo getApiSixRouteInfo(DasApiBasicInfo dasApiBasicInfo,
                                             String upstreamId,
                                             String serviceId,
                                             DasApiRegister dasApiRegister) {
  ...
  private Map<String, List<DasApiBasicInfo>> getDasApiBasicInfoByRegisterType() {
    Map<String, List<DasApiBasicInfo>> dasApiBasicInfoMap = new HashMap<>(16);
    //TODO 策略方式是否更新 获取所有注册API基础信息
    Iterable<DasApiBasicInfo> apiBasicInfoIterable =
        dasApiBasicInfoRepository.findAll(qDasApiBasicInfo.apiType.eq(ApiTypeEnum.REGISTER_API.getCode()));
