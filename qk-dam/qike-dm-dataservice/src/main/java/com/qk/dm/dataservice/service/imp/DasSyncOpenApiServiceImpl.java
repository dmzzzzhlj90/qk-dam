...
public class DasSyncOpenApiServiceImpl implements DasSyncOpenApiService {
    ...
    private void getDasApiRegister(List<DasApiRegisterVO> dasApiRegisterVOList, DasApiDirVO dasApiDirVO, OpenApi3 openApi3) {
        ...
        for (String pathKey : apiMap.keySet()) {
            ...
            apiBasicInfoVOBuilder
                    .dirId(dasApiDirVO.getDirId())
                    //TODO Level
                    .dirName(dasApiDirVO.getTitle());
            // 构建注册Api对象
            DasApiRegisterVO dasApiRegisterVO =
                    buildRegisterVO(apiMap, pathKey, apiBasicInfoVOBuilder, openApi3);
    ...
    private DasApiRegisterVO buildRegisterVO(Map<String, Path> apiMap,
                                             String pathKey,
                                             DasApiBasicInfoVO.DasApiBasicInfoVOBuilder apiBasicInfoVOBuilder,
                                             OpenApi3 openApi3) {
        ...
        DasApiRegisterDefinitionVO.DasApiRegisterDefinitionVOBuilder dasApiRegisterDefinitionVOBuilder = DasApiRegisterDefinitionVO.builder();
        // OpenApi中的参数获取
        ...
        for (Map.Entry<String, Operation> operationEntry : path.getOperations().entrySet()) {
            ...
            dasApiRegisterDefinitionVOBuilder
                    .backendHost(urlHost)
                    .backendPath(pathKey)
            ...
            // 同步入参信息
            existRequestBody(schemaMap, apiBasicInfoVOBuilder, dasApiRegisterDefinitionVOBuilder, requestBody);
            // 构建API基础信息
            DasApiBasicInfoVO dasApiBasicInfoVO = apiBasicInfoVOBuilder.build();
            apiRegisterVOBuilder.dasApiBasicInfoVO(dasApiBasicInfoVO);
            // 构建注册API
            DasApiRegisterDefinitionVO dasApiRegisterDefinitionVO = dasApiRegisterDefinitionVOBuilder.build();
            apiRegisterVOBuilder.dasApiRegisterDefinitionVO(dasApiRegisterDefinitionVO);
